...
export function createBasicTable(employee_data) {
  ...
  const employeeData = employee_data || getData().employeeData;
  ...
  for (const rows of employeeData) {
    ...
    for (const cols in headerArrToCompare) {
      ...
      if (headerArrToCompare[cols] === 'Actions') {
        // update button
        const updateBtn = createUpdateButton(rows['userId']);
...
export function fillDataIntoForm(userId) {
...
function createUpdateButton(userId) {
  ...
  updateBtn.addEventListener('click', () => {
    ...
    // fillRowDataIntoFormChannel.postMessage({ userId: userId });
...
function createBasicDeleteButton(tr, userId) {
  ...
  deleteBtn.addEventListener('click', () => {
    ...
    // deleteRowChannel.postMessage({ rowIndex: rowIndex, userId: userId });
