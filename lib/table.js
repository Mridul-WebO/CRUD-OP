  ...
  for (const rows of employeeData) {
    ...
    for (const cols in headerArr) {
      ...
      if (headerArr[cols] === 'Actions') {
        ...
        td.innerText = Array.isArray(rows[headerArr[cols]])
          ? rows[headerArr[cols]].length === 0
            ? '-'
            : rows[headerArr[cols]]
          : rows[headerArr[cols]];
      }
...
export function addRow(submittedData) {
  ...
  for (const cols in submittedData) {
    ...
    td.innerText = Array.isArray(submittedData[cols])
      ? submittedData[cols].length === 0
        ? '-'
        : submittedData[cols]
      : submittedData[cols];
  ...
  const updateBtn = createUpdateButton(tr, submittedData['userId']);
  td.appendChild(updateBtn);
  ...
  for (const headers in headerArr) {
    ...
    for (const value of employeeData) {
      ...
      if (headerArr[headers] === 'Actions') {
        ...
        childTd.innerText = Array.isArray(value[headerArr[headers]])
          ? value[headerArr[headers]].length === 0
            ? '-'
            : value[headerArr[headers]]
          : value[headerArr[headers]];
      }
...
export function addRowToAdvTable(employeeData) {
  ...
  trs.forEach((val, index) => {
    ...
    if (headerArr[index] === 'Actions') {
      ...
      childTd.innerText = employeeData[headerArr[index]].length === 0 ? '-' : employeeData[headerArr[index]];
    }
